...
in
{
          ...
          { name, ... }:
          {
            options = {
              ...
              carrier = lib.mkOption {
                ...
                description = "The WireGuard carrier, which only ever carries the GRETAP.";
              };
              publicKey = lib.mkOption {
                type = lib.types.str;
                description = "The far site's WireGuard public key.";
              };
              ...
              port = lib.mkOption {
                type = lib.types.port;
                description = "Our WireGuard listen port for this link, opened on the WANs by nftables.";
              };
              localAddress = lib.mkOption {
                type = lib.types.str;
                description = ''
                  Our address on the carrier with its prefix length, one end
                  of a /127. The GRETAP is built on these two addresses and
              ...
              mtu = lib.mkOption {
                ...
                default = 1382;
                description = ''
                  Link MTU: the carrier less GRETAP's 38 (outer IPv4 20,
                  GRE 4, inner ethernet 14). An IGP must be told rather
                  than left on defaults sized for 1500: FRR's isisd, for
                  one, defaults lsp-mtu to 1497, which does not fit, and
                  generates LSPs too large to flood.
                '';
              };
            };
          }
  ...
  config = lib.mkIf (cfg.links != { }) {
    assertions = [
      {
        ...
        message = "homelab.interconnect.links needs privateKeyFile for the WireGuard carriers";
      }
      {
        assertion =
          let
            ports = lib.mapAttrsToList (_: link: link.port) cfg.links;
          in
    ...
    systemd.network = {
      netdevs = lib.concatMapAttrs (_: link: {
        "45-${link.carrier}" = {
          netdevConfig = {
            Name = link.carrier;
            Kind = "wireguard";
      ...
      networks = lib.concatMapAttrs (_: link: {
        "45-${link.carrier}" = {
