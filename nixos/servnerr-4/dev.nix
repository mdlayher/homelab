    ...
    text = ''
      ...
      case $verb in
        ...
        edit)
          ...
          ;;
        # Onboarding a machine adds a recipient, which is the one sops
        # operation an edit does not cover: the file has to be re-encrypted
        # to the group .sops.yaml now names. sops prints the diff and asks
        # before writing, on the tty the sudo below passes through.
        updatekeys)
          [[ $# -eq 1 ]] || usage
          sops updatekeys "$1"
          ;;
