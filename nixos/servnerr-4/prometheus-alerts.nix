  ...
  groups = [
    {
      ...
      rules = [
        ...
        }
        {
          ...
          for = "1m";
          annotations.summary = "TLS certificate for {{ $labels.instance }} will expire in less than 2 days.";
        }
        {
          alert = "DiskUsageHigh";
        ...
        # have IPv6 autoconfiguration disabled.
        {
          ...
          expr = ''(corerad_interface_advertising{job="corerad"} == 1) and ((corerad_interface_forwarding == 0) or (corerad_interface_autoconfiguration == 1))'';
          for = "1m";
        ...
        # so hosts don't drop their default route.
        {
          ...
          expr = ''rate(corerad_advertiser_router_advertisements_total{job="corerad",type="multicast"}[20m]) == 0'';
          for = "1m";
        ...
        # Monitor for inconsistent advertisements from hosts on the LAN.
        {
          ...
          expr = ''rate(corerad_advertiser_router_advertisement_inconsistencies_total{job="corerad"}[5m]) > 0'';
        ...
        # static /40) and ULA /48.
        {
          ...
          expr = ''(count by(instance, interface) (corerad_advertiser_prefix_autonomous{job="corerad",prefix=~"2600:6c4a:78.*|fd9e:1a04:f01d:.*"} == 1) != bool 2) == 1'';
          for = "1m";
        ...
        # Expect regular upstream router advertisements.
        {
          ...
          expr = ''changes(corerad_monitor_messages_received_total{job="corerad",message="router advertisement"}[30m]) == 0'';
        ...
        # much lower threshold.
        {
          ...
          expr = ''corerad_monitor_default_route_expiration_timestamp_seconds{instance=~"routnerr-.*",job="corerad"} - time() < 2*60*60'';
        ...
        }
        {
          ...
          expr = ''corerad_monitor_default_route_expiration_timestamp_seconds{instance!~"routnerr-.*",job="corerad"} - time() < 1*60*10'';
